...
fn solve(base_alloc: std.mem.Allocator, input_str: []u8) !Solution {
    ...
    while (lines.next()) |line| {
        ...
        const was_positive = position > 0;
        const total_shift = instruction.to_i16();
        const div100 = @divTrunc(total_shift, 100);
        const mod100 = @rem(total_shift, 100);
        n_zeros += @abs(div100);
        position += mod100;
        if (position <= 0 and was_positive) {
            n_zeros += 1;
