...
fn solve(n_boxes: comptime_int, n_connections: comptime_int, input_str: []u8) !Solution {
    ...
    const shortest_connections = argmin_n(n_connections, &sqdists.data);
    ...
    var sizes_mirrored = [_]usize{math.maxInt(usize)} ** n_boxes;
    for (circuits) |c| {
        sizes_mirrored[c] -= 1;
    }
    const argmax_sizes = argmin_n(3, &sizes_mirrored);
    var prod: Solution = 1;
    for (argmax_sizes) |i| {
        const size = math.maxInt(usize) - sizes_mirrored[i];
        prod *= @intCast(size);
    }
    return prod;
}
...
pub fn main() !void {
    ...
    const solution = try solve(n_positions, n_connections, fileContent);
...
test "Example" {
    ...
    const sum = try solve(n_positions, n_connections, fileContent);
...
test "Benchmark" {
    ...
    const n_positions = 1000;
    const n_connections = 1000;
    _ = try solve(n_positions, n_connections, fileContent);
}
